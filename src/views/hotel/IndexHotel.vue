            ...
            <div class="col-md-12">
                <div class="card border-0 rounded shadow">
                    <div class="card-body">
                        <h4>DATA POST</h4>
                        <hr />
                        <router-link :to="{ name: 'hotel.create' }" class="btn btn-md btn-success">Tambah Hotel
                        </router-link>
                        ...
                        <table class="table table-striped table-bordered mt-4">
                            ...
                            <tbody>
                                <tr v-for="(post, index) in posts" :key="index">
                                    <td>{{ index + 1 }}</td>
                                    <td>{{ post.nama_hotel }}</td>
                                    <td>{{ post.alamat }}</td>
                                    <td>
                                        <router-link :to="{ name: 'hotel.edit', params: { id: post.id } }"
    ...
    export default {
        setup() {
            ...
            function postDelete(id) {
                ...
                axios.delete(`http://localhost:8000/api/hotel/${id}`)
                    .then(() => {
